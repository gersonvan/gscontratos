  ...
  def procurement_rows_table1
    [
      [{:content => 'DADOS GERAIS', :colspan => 10}],
        [{:content => 'CLIENTE: '"\n" + @procurement.cliente.to_s, :colspan => 4},
          {:content => 'TELEFONE: '"\n" + @procurement.fones.to_s + ", " + @procurement.fone2.to_s + ", " + @procurement.fone3.to_s, :colspan => 2},
          {:content => 'E. CIVIL: '"\n" + @procurement.estado_civil.to_s, :colspan =>1},
          {:content => 'RG: '"\n" + @procurement.rg.to_s, :colspan => 2},
          {:content => 'CPF: '"\n" + @procurement.cpf.to_s, :colspan => 1}],
        [{:content => 'ENDEREÇO: '"\n" + @procurement.endereco.to_s, :colspan => 3},
          {:content => 'BAIRRO: '"\n" + @procurement.bairro.to_s, :colspan => 2},
          {:content => 'CEP: '"\n" + @procurement.cep_usuario.to_s, :colspan => 1},
          {:content => 'CIDADE: '"\n" + @procurement.cidade.to_s, :colspan => 2},
          {:content => 'NACIONALIDADE: '"\n" + @procurement.nacionalidade.to_s, :colspan => 2}],
        [{:content => 'PARTE CONTRÁRIA: '"\n" + @procurement.parte_contraria.to_s, :colspan => 5},
          {:content => 'CNPJ: '"\n" + @procurement.cnpj.to_s, :colspan => 1},
          {:content => 'NOME FANTASIA: '"\n" + @procurement.nome_fantasia.to_s, :colspan =>4}],
        ...
        [{:content => 'PROPRIETÁRIO: '"\n" + @procurement.proprietario_empresa.to_s, :colspan => 3},
          {:content => 'CEP: '"\n" + @procurement.cep_empresa.to_s, :colspan => 1},
          {:content => 'RAMO DE ATIVIDADE: '"\n" + @procurement.ramo_de_atividade.to_s, :colspan => 2},
          {:content => 'OBJETO DE AÇÃO: '"\n" + @procurement.objeto_de_acao.to_s, :colspan => 4}],
     ]
  end
